...
go mod download github.com/authzed/grpcutil
go mod tidy
```
...
resp = client.schema_service.write_schema(
  ...
  <TabItem value="node">
<SampleCodeBlock lang="ts">{`import {
  ClientSecurity,
  NewClient,
  ObjectReference,
  SubjectReference,
  ...
  Relationship,
  RelationshipUpdate,
} from from '@authzed/authzed-node/v1';
·
...
·
const emilia = ObjectReference.create({
  objectType: '{{ tenant }}/user',
  objectId: 'emilia',
});
·
const beatrice = ObjectReference.create({
  objectType: '{{ tenant }}/user',
  objectId: 'beatrice',
});
·
...
·
console.log(response)`}</SampleCodeBlock>
  </TabItem>
</Tabs>
...
·
const emiliaCanRead = await client.checkPermission(CheckPermissionRequest.create({
  resource,
  permission: 'read',
  subject: SubjectReference.create({
    object: emilia,
...
console.log(emiliaCanRead.permissionship === CheckPermissionResponse_Permissionship.HAS_PERMISSION);
·
const emiliaCanWrite = await client.checkPermission(CheckPermissionRequest.create({
  resource,
  permission: 'write',
  subject: SubjectReference.create({
    object: emilia,
...
console.log(emiliaCanWrite.permissionship === CheckPermissionResponse_Permissionship.HAS_PERMISSION);
·
const beatriceCanRead = await client.checkPermission(CheckPermissionRequest.create({
  resource,
  permission: 'read',
  subject: SubjectReference.create({
    object: beatrice,
...
console.log(beatriceCanRead.permissionship === CheckPermissionResponse_Permissionship.HAS_PERMISSION);
·
const beatriceCanWrite = await client.checkPermission(CheckPermissionRequest.create({
  resource,
  permission: 'write',
  subject: SubjectReference.create({
    object: beatrice,
...
console.log(beatriceCanWrite.permissionship === CheckPermissionResponse_Permissionship.HAS_PERMISSION);`}</SampleCodeBlock>
  </TabItem>
</Tabs>
