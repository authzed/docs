...
In fact, the paper that inspired SpiceDB is entitled "Zanzibar: Google's \***\*Consistent\*\***, Global Authorization System".
...
`at_exact_snapshot` will ensure that all data used for computing the response is that found at the _exact_ point-in-time specified in the [ZedToken].
...
Note that the snapshot used will be loaded at the beginning of the API call, and that new data written _after_ the API starts executing will be ignored.
