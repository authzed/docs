...
Materialize puts that precomputed data to work in two ways: **Accelerated Queries**, which is generally available, and **Event Streams**, which is in early access. Every other piece of Materialize content belongs to one of these two features — look for the badge at the top of the page, which also shows its release status.

## Accelerated Queries

<Callout type="info">Accelerated Queries is generally available.</Callout>
