...
<Tabs items={["Node", "Go", "Python", "Ruby", "Java", ".NET"]}>
  ...
  <Tabs.Tab>
    `ChannelCredentials.Insecure` with `UnsafeUseInsecureChannelCallCredentials = true` (also
    requires `AppContext.SetSwitch("System.Net.Http.SocketsHttpHandler.Http2UnencryptedSupport",
    true)`)
  </Tabs.Tab>
</Tabs>
