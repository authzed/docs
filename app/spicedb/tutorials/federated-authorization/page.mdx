...
<Callout type="warning">
  Note: Key in on the immutable id but never the email. Emails change and get reassigned, so an
  email-keyed binding can silently point at the wrong person.
</Callout>

<Callout type="warning">
  If the lookup errors, fail the login otherwise the system can swallow the error and create a new
  user instead, and a returning user loses all their access.
</Callout>
