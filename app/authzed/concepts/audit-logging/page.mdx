...
<Callout type="warning">
  **AuthZed Enterprise only.** The `--extender-audit-*` flags below exist in the AuthZed-licensed SpiceDB Enterprise binary.
  They are not present in [SpiceDB open source](/authzed/guides/picking-a-product#open-source), which has no audit logging.
</Callout>
