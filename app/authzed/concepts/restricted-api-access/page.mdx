...
<ProductBadge
  available={["cloud", "dedicated", "enterprise"]}
  diy={["open-source"]}
/>
...
<Callout type="warning">
  **AuthZed Enterprise only.** The `--extender-*` flags below exist in the
  AuthZed-licensed SpiceDB Enterprise binary. They are not present in [SpiceDB
  open source](/authzed/guides/picking-a-product#open-source).
</Callout>
