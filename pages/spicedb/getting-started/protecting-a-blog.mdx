...
When connecting to SpiceDB running locally without TLS, use `INSECURE_PLAINTEXT_CREDENTIALS` (Node.js), `insecure_bearer_token_credentials` (Python), or the equivalent insecure option for your client library. This is the recommended setting for **any** local development scenario, including:
- Localhost connections
- Docker containers (including Docker Desktop)
- Orbstack with `*.orb.local` hostnames
- Remote Kubernetes port-forwards
- Any other non-TLS local setup

Note: `INSECURE_LOCALHOST_ALLOWED` (if available in your client) works for true localhost connections and port-forwards, but will fail when connecting to Docker containers or Orbstack with non-localhost hostnames like `*.orb.local`. For broader compatibility across all local development scenarios, use `INSECURE_PLAINTEXT_CREDENTIALS` instead.

**Important:** Always switch back to TLS-secured credentials (e.g., `bearer_token_credentials`) before deploying to production.
</Callout>
...
const { promises: client } = v1.NewClient(
  't_your_token_here_1234567deadbeef',
  'grpc.authzed.com:50051',
  // For local development without TLS (localhost, Docker, Orbstack, etc.)
