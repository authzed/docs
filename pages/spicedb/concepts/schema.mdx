...
A SpiceDB schema defines the types of objects found, how those objects relate to one another, and the permissions that can be computed from those relations.
...
Caveats are a feature of SpiceDB that allows for relationships to be defined conditionally: the relationship will only be considered present if the caveat expression evaluates to true at query time.
...
Relations can also specify wildcards to indicate that access can be granted to the resource _type_ as a whole, rather than a particular resource.
...
For example, a permission would determine whether a user within the set of users that can `edit` a document.
...
A union operation _unions together_ relations/permissions in an expression.

Union is the most common operation and joins together different relations or permissions together to form a set of allowed subjects.
...
An intersection operation _intersects the set of subjects_ found for the relations/permissions in an expression.

Permissions defined using intersections only include subjects found in **both** relations/permissions.
...
An exclusion operation _excludes the set of subjects_ found for the right side relation/permission from those found in the left side relation/permission.

Exclusions compute the difference between two sets of relations/permissions.
